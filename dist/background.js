...
Standards{linkEnd}.`,
    ...
    chrome.storage.local.get(null, (currentSettings) => {
      ...
      for (let [key, defaultValue] of Object.entries(defaults)) {
        ...
        if (storedValue === void 0)
          ...
          storedValue === null ? (console.warn(`Mania Ext: Setting '${key}' was null but expected ${defaultType}. Resetting.`), settingsToUpdate[key] = defaultValue, needsUpdate = !0) : storedType !== defaultType && (console.warn(`Mania Ext: Type mismatch for '${key}'. Expected ${defaultType}, got ${storedType}. Resetting.`), settingsToUpdate[key] = defaultValue, needsUpdate = !0);
        }
      }
      needsUpdate && chrome.storage.local.set(settingsToUpdate, () => {
        chrome.runtime.lastError ? console.error("Mania Ext: Failed to sync settings.", chrome.runtime.lastError) : console.log(`Mania Ext: Synced/Fixed ${Object.keys(settingsToUpdate).length} settings (Trigger: ${reason}).`);
      });
    });
  }
  ...
  async function setupContextMenuListener() {
    await chrome.permissions.contains({ permissions: ["contextMenus"] }) && chrome.contextMenus && !chrome.contextMenus.onClicked.hasListener(contextMenuClickListener) && chrome.contextMenus.onClicked.addListener(contextMenuClickListener);
  }
  ...
  async function getUniverseIdFromPlaceId(placeId) {
    try {
      ...
      return console.error("Mania Ext: Error fetching universe ID from place ID", e), null;
    }
  }
  __name(getUniverseIdFromPlaceId, "getUniverseIdFromPlaceId");
  async function callRobloxApiBackground(options) {
    let { subdomain = "api", endpoint, method = "GET", body = null, headers = {} } = options, separator = endpoint.includes("?") ? "&" : "?", url = `https://${subdomain}.roblox.com${endpoint}${separator}_RoValraRequest=`, fetchOptions = { method, headers: { ...headers } };
  ...
  async function wearOutfit(outfitId) {
    let callWithRetry = /* @__PURE__ */ __name(async (options) => {
      for (let i = 0; i < 4; i++) {
        let response = await callRobloxApiBackground(options);
    ...
    chrome.storage.local.get(["rovalra_avatar_rotator_enabled", "rovalra_avatar_rotator_ids", "rovalra_avatar_rotator_interval"], (data) => {
      if (state.rotatorInterval && (clearInterval(state.rotatorInterval), state.rotatorInterval = null), data.rovalra_avatar_rotator_enabled && data.rovalra_avatar_rotator_ids?.length > 0) {
        ...
        let intervalSeconds = Math.max(parseInt(data.rovalra_avatar_rotator_interval, 10) || 5, 5), rotate = /* @__PURE__ */ __name(() => {
          ...
          let outfitId = ids[state.rotatorIndex];
          wearOutfit(outfitId), state.rotatorIndex = (state.rotatorIndex + 1) % ids.length;
        }, "rotate");
  ...
  chrome.permissions.onAdded.addListener((permissions) => {
    permissions.permissions?.includes("webNavigation") && setupNavigationListener(), permissions.permissions?.includes("contextMenus") && setupContextMenuListener(), chrome.tabs.query({}, (tabs) => {
      tabs.forEach((tab) => chrome.tabs.sendMessage(tab.id, { action: "permissionsUpdated" }).catch(() => {
      }));
    });
  });
  chrome.permissions.onRemoved.addListener((permissions) => {
    permissions.permissions?.includes("webNavigation") && chrome.webNavigation.onBeforeNavigate.hasListener(navigationListener) && chrome.webNavigation.onBeforeNavigate.removeListener(navigationListener), permissions.permissions?.includes("contextMenus") && chrome.contextMenus?.onClicked.hasListener(contextMenuClickListener) && chrome.contextMenus.onClicked.removeListener(contextMenuClickListener), chrome.tabs.query({}, (tabs) => {
      tabs.forEach((tab) => chrome.tabs.sendMessage(tab.id, { action: "permissionsUpdated" }).catch(() => {
      }));
    });
  });
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    switch (request.action) {
      ...
      case "requestPermission":
        return chrome.permissions.request({ permissions: [].concat(request.permission) }, (granted) => {
          chrome.runtime.lastError && console.warn("Mania Ext: Permission request failed:", chrome.runtime.lastError), sendResponse({ granted: !!granted });
        }), !0;
      ...
      case "wearOutfit":
        return wearOutfit(request.outfitId).then((result) => sendResponse(result)), !0;
      case "updateContextMenu":
        return chrome.contextMenus && chrome.storage.local.get(["copyIdEnabled", "copyUniverseIdEnabled"], (settings) => {
